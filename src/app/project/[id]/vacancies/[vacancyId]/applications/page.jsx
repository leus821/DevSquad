...
const VacancyApplicationsPage = () => {
	...
	const handleAccept = async candidate => {
		...
		try {
			...
			const currentMembers = Array.isArray(projectData.members)
				? projectData.members
				: [];
			...
			<Container>
				...
				<div className='mb-10'>
					...
					<p className='text-header-icons mt-1'>
						Роль:{' '}
						<span className='text-brand-purple font-bold'>{vacancy?.role}</span>
					</p>
				</div>
				...
				<div className='grid gap-4'>
					{profiles.length > 0 ? (
						profiles.map(user => (
							<div
								key={user.id}
								className='card p-6 flex flex-col sm:flex-row justify-between items-center gap-6'
							>
								<div className='flex items-center gap-4 w-full'>
									...
									<div className='min-w-0'>
										...
										<p className='text-sm text-header-icons'>
											@{user.username}
										</p>
										<Link
											href={`/user/${user.id}`}
